"""物料演示的默认子工作流（host 启动时幂等上报，经管理 API 运行）。
...
def site_loop(ctx: WorkflowBuildContext) -> None:
    ...
    ctx.run_template("sample_rack_demo/verify_site_definition", {}, name="核对位点声明")
    ctx.run("sample_rack/load_sample", {"site": "A1", "sample_name": "proof-sample"}, name="装载样品")
    ctx.run("sample_rack/transfer_sample", {"from_label": "A1", "to_label": "B2"}, name="转移样品")
    ctx.run("sample_rack/inspect_sites", {}, name="复查位点")
...
def material_loop(ctx: WorkflowBuildContext) -> None:
    ...
    ctx.run("material_bench/prepare_bench", {}, name="准备台面")
    ctx.run(
    ...
    ctx.run("material_bench/relocate_plate", {"to_site": "T3"}, name="转移板位")
